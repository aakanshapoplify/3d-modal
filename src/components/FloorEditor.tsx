...
function assignMaterial(mesh: THREE.Mesh, boundingBox: THREE.Box3, wallColor = "#FFFFFF", forceWallMode = false) {
...
function FloorGLB({ url, wallColor, forceWallMode }: { url: string; wallColor: string; forceWallMode: boolean }) {
...
export default function FloorEditor() {
  ...
  const [floorUrl, setFloorUrl] = useState("/models/floor.glb");
  const [isLoadingFloor, setIsLoadingFloor] = useState(false);
  const [uploadedFloor, setUploadedFloor] = useState<string | null>(null);
  ...
  const addFurnitureAt = (point: { x: number; y: number; z: number }) => {
  ...
  const handleFloorUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // For now, we'll use the existing obj-to-glb endpoint
      // In a real implementation, you'd have specific endpoints for JPG/DXF conversion
      const response = await fetch('/api/obj-to-glb', {
        method: 'POST',
        body: formData,
      });
  ...
  const nudge = (axis: 'x' | 'z', direction: number) => {
  ...
  const scale = (direction: number) => {
    const item = placed.find(i => i.id === selectedId);
    if (!item) return;
    
    const factor = direction > 0 ? 1.1 : 0.9;
    const newScale = item.scale.map(s => s * factor) as [number, number, number];
